 ...
 * @param {RegExpMatchArray} match - match[1] é o slug da classe
 */
export async function renderClassPage(outlet, match) {
  ...
  section.innerHTML = `
    <nav style="padding:var(--spacing-md,1rem) 0;font-size:0.875rem;color:var(--color-text-muted,#6b5a4a);">
      <a href="/" data-nav>Início</a> / <span>${cls.name}</span>
    </nav>
  `;
  ...
  header.innerHTML = `
    ...
    ${cls.image ? `<figure style="margin:var(--spacing-lg,1.5rem) auto;max-width:400px;">
      <img src="${cls.image}" alt="${cls.name}" style="width:100%;height:auto;border-radius:var(--radius-md,8px);box-shadow:0 4px 12px var(--color-shadow,rgba(44,24,16,0.1));" loading="lazy" />
    </figure>` : ''}
  ...
  if (cls.progressionTable && cls.progressionTable.columns && cls.progressionTable.rows) {
  ...
  if (cls.features && cls.features.length > 0) {
    ...
    featuresSection.innerHTML = `<h2 style="font-family:var(--font-display,'Cinzel',serif);color:var(--color-primary,#8b0000);margin:var(--spacing-xl,2rem) 0 var(--spacing-md,1rem);">Características de Classe</h2>`;
    ...
    for (const feature of cls.features) {
      ...
      article.innerHTML = `
        <h3 style="font-family:var(--font-display,'Cinzel',serif);color:var(--color-accent,#daa520);margin-bottom:var(--spacing-sm,0.5rem);">
          ${feature.name || ''}
          ${feature.level ? `<span style="font-size:0.8rem;color:var(--color-text-muted,#6b5a4a);font-weight:400;">(nível ${feature.level})</span>` : ''}
        </h3>
        <div style="line-height:1.6;">${feature.description || ''}</div>
      `;
  ...
  if (cls.subclasses && cls.subclasses.length > 0) {
    ...
    subSection.innerHTML = `
      <h2 style="font-family:var(--font-display,'Cinzel',serif);color:var(--color-primary,#8b0000);margin:var(--spacing-xl,2rem) 0 var(--spacing-md,1rem);">Subclasses</h2>
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(250px,1fr));gap:var(--spacing-md,1rem);">
        ${cls.subclasses.map(sc => `
          <div style="background:var(--color-surface,#fff8ec);border:1px solid var(--color-border,#d4c4a8);border-radius:var(--radius-md,8px);padding:var(--spacing-md,1rem);">
            <h3 style="font-family:var(--font-display,'Cinzel',serif);font-size:1rem;color:var(--color-accent,#daa520);">${sc.name}</h3>
            ${sc.source ? `<span style="font-size:0.75rem;color:var(--color-text-muted,#6b5a4a);">${sc.source}</span>` : ''}
          </div>
        `).join('')}
      </div>
    `;
